...
{
    switch (m_type) {
    [[likely]] case Kinesis::Stochastic::Algorithm::UNIFORM:
        return m_current_start + fast_uniform() * (m_current_end - m_current_start);

    case Kinesis::Stochastic::Algorithm::NORMAL:
        rebuild_distributions_if_needed();
        return m_normal_dist(m_random_engine);

    case Kinesis::Stochastic::Algorithm::EXPONENTIAL:
        rebuild_distributions_if_needed();
        return m_exponential_dist(m_random_engine);

    case Kinesis::Stochastic::Algorithm::POISSON: {
...
{
    if (m_type == Kinesis::Stochastic::Algorithm::NORMAL) {
        sample = std::max(start, std::min(end, sample));
    } else if (m_type == Kinesis::Stochastic::Algorithm::EXPONENTIAL) {
        sample /= end;
