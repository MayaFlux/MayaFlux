    ...
    const std::shared_ptr<Buffers::VKBuffer>& staging)
{
    if (!is_initialized() || !image || !data || !staging) {
        MF_ERROR(Journal::Component::Portal, Journal::Context::ImageProcessing,
            "download_data(staging): invalid parameters");
        return;
    }
    ...
    if (!buffer_service || !buffer_service->execute_fenced
        ...
        MF_ERROR(Journal::Component::Portal, Journal::Context::ImageProcessing,
            "download_data(staging): BufferService unavailable or incomplete");
        return;
    }
    ...
    if (!handle) {
        MF_ERROR(Journal::Component::Portal, Journal::Context::ImageProcessing,
            "download_data(staging): execute_fenced returned null handle");
        return;
    }
    ...
    if (mapped) {
        ...
        MF_ERROR(Journal::Component::Portal, Journal::Context::ImageProcessing,
            "download_data(staging): staging buffer has no mapped pointer");
    }
