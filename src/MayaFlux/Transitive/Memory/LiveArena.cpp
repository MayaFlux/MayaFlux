...
namespace internal {
    ...
    std::atomic<bool> g_live_exposure_enabled { false };

    void enable_live_exposure() noexcept
    {
        g_live_exposure_enabled.store(true, std::memory_order_relaxed);
    }

    bool live_exposure_enabled() noexcept
    {
        return g_live_exposure_enabled.load(std::memory_order_relaxed);
    }
