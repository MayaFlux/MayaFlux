...
RelaxationGridBuffer::RelaxationGridBuffer(
    ...
    , m_rule_shader_path(std::move(rule_shader_path))
    , m_emit_shader_path(std::move(emit_shader_path))
{
...
RelaxationGridBuffer::RelaxationGridBuffer(
    ...
    , m_height(height)
    , m_cell_stride_bytes(cell_stride_bytes)
    , m_rule_spec(std::move(rule_spec))
    , m_emit_spec(std::move(emit_spec))
    , m_uses_spec_construction(true)
{
    auto buffer_service = Registry::BackendRegistry::instance()
                              .get_service<Registry::Service::BufferService>();
...
void RelaxationGridBuffer::setup_processors(ProcessingToken token)
{
    ...
    auto step_processor = m_uses_spec_construction
        ? std::make_shared<RelaxationStepProcessor>(m_rule_spec)
        : std::make_shared<RelaxationStepProcessor>(m_rule_shader_path);
    ...
    auto emit_processor = m_uses_spec_construction
        ? std::make_shared<RelaxationEmitProcessor>(m_emit_spec)
        : std::make_shared<RelaxationEmitProcessor>(m_emit_shader_path);
