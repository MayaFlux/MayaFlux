...
void RelaxationGridBuffer::setup_processors(ProcessingToken token)
{
    ...
    auto step_processor = std::make_shared<RelaxationStepProcessor>(m_rule_shader_path);
    ...
    auto emit_processor = std::make_shared<RelaxationEmitProcessor>(m_emit_shader_path);
