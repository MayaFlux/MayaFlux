...
void MeshNetworkProcessor::on_attach(const std::shared_ptr<Buffer>& buffer)
{
    ...
    if (!m_buffer_service) {
        error<std::runtime_error>(
            Journal::Component::Buffers, Journal::Context::BufferProcessing,
            std::source_location::current(),
            "MeshNetworkProcessor requires a valid BufferService");
    }
    ...
    if (!vk_buf->is_initialized()) {
        m_buffer_service->initialize_buffer(vk_buf);
    }
