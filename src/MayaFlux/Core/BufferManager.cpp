    ...
    , m_global_processing_chain(std::make_shared<Buffers::BufferProcessingChain>())
{
    ...
    for (u_int32_t i = 0; i < num_channels; i++) {
        ...
        auto buffer { std::make_shared<Buffers::RootAudioBuffer>(i, num_frames) };
...
void BufferManager::add_buffer_to_channel(u_int32_t channel_index, std::shared_ptr<Buffers::AudioBuffer> buffer)
{
    if (channel_index >= m_num_channels) {
        throw std::out_of_range("Channel index out of range");
    }
...
void BufferManager::remove_buffer_from_channel(u_int32_t channel_index, std::shared_ptr<Buffers::AudioBuffer> buffer)
{
    if (channel_index >= m_num_channels) {
        throw std::out_of_range("Channel index out of range");
    }
...
const std::vector<std::shared_ptr<Buffers::AudioBuffer>>& BufferManager::get_channel_buffers(u_int32_t channel_index) const
{
    if (channel_index >= m_num_channels) {
        throw std::out_of_range("Channel index out of range");
    }
...
void BufferManager::add_processor(std::shared_ptr<Buffers::BufferProcessor> processor, std::shared_ptr<Buffers::AudioBuffer> buffer)
{
    u_int32_t channel_id = buffer->get_channel_id();

    if (channel_id < m_num_channels) {
        std::shared_ptr<Buffers::BufferProcessingChain> chain = m_channel_processing_chains[channel_id];
        chain->add_processor(processor, buffer);
        buffer->set_processing_chain(chain);
    }
}

void BufferManager::add_processor_to_channel(std::shared_ptr<Buffers::BufferProcessor> processor, u_int32_t channel_index)
{
    if (channel_index >= m_num_channels) {
        throw std::out_of_range("Channel index out of range");
    }
...
void BufferManager::remove_processor(std::shared_ptr<Buffers::BufferProcessor> processor, std::shared_ptr<Buffers::AudioBuffer> buffer)
{
    std::shared_ptr<Buffers::BufferProcessingChain> chain = m_channel_processing_chains[buffer->get_channel_id()];

    chain->remove_processor(processor, buffer);
}

void BufferManager::remove_processor_from_channel(std::shared_ptr<Buffers::BufferProcessor> processor, u_int32_t channel_index)
{
    if (channel_index >= m_num_channels) {
        throw std::out_of_range("Channel index out of range");
    }
...
void BufferManager::connect_node_to_buffer(std::shared_ptr<Nodes::Node> node, std::shared_ptr<Buffers::AudioBuffer> buffer, float mix, bool clear_before)
{
    auto processor = std::make_shared<Buffers::NodeSourceProcessor>(node, mix, clear_before);
