...
public:
    ...
    output_type apply_operation(const input_type& input)
    {
        ...
        return apply_operation_internal(input, ctx);
    }
    ...
    {
        ExecutionContext dep_ctx { .mode = ExecutionMode::DEPENDENCY };

        for (auto& dep : m_dependencies) {
            if (dep->validate_input(input)) {
                dep->apply_operation_internal(input, dep_ctx);
            }
        }

        ExecutionContext main_ctx { .mode = ExecutionMode::SYNC };
        return apply_operation_internal(input, main_ctx);
    }
    ...
    {
        ExecutionContext ctx { .mode = ExecutionMode::DEPENDENCY };
        return apply_operation_internal(input, ctx);
    }
...
protected:
     ...
     */
    virtual output_type apply_operation_parallel(const input_type& input, const ExecutionContext& /*context*/)
    {
        return operation_function(input);
    }
