...
/**
 * @brief Windowing transformation using C++20 ranges
    ...
    bool in_place = false)
{
    ...
    uint32_t size = window_size > 0 ? window_size : static_cast<uint32_t>(target_data.size());

    auto window = Nodes::Generator::generate_window(size, window_type);

    auto data_view = target_data | std::views::take(std::min(size, static_cast<uint32_t>(target_data.size())));
...
/**
 * @brief Spectral filtering using existing FFT infrastructure with C++20 ranges
 * @tparam DataType ComputeData type
 * @param input Input data
...
/**
 * @brief Pitch shifting using existing FFT from AnalysisHelper with C++20 ranges
 * @tparam DataType ComputeData type
 * @param input Input data
...
/**
 * @brief Spectral inversion (phase inversion in frequency domain) using C++20 ranges
 * @tparam DataType ComputeData type
 * @param input Input data
 * @param window_size FFT window size
 * @param hop_size Hop size for overlap-add
