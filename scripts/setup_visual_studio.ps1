...
& (Join-Path $VCPKG_DIR "vcpkg") install --triplet x64-windows llvm rtaudio vulkan ffmpeg gtest eigen3 magic-enum glfw3
if (-not $?) {
...
if (-not ($installed -match "llvm")) { Write-Host "Warning: llvm may not be installed correctly." }
