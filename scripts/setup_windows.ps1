...
& "$vcpkgDir\vcpkg.exe" install --triplet x64-windows llvm rtaudio vulkan ffmpeg gtest eigen3 magic-enum glfw3
if ($LASTEXITCODE -ne 0) {
...
& "$vcpkgDir\vcpkg.exe" list | Where-Object { $_ -match "llvm|rtaudio|vulkan|ffmpeg|gtest|eigen3|magic_enum|glfw3" }
if ($LASTEXITCODE -ne 0) {
